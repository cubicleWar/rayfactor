...
void Triangle::traceFactors(Primitive *head, VFMatrix* vfm)
{
    ...
    //const Vectorx4 wsNormal(_mm_set1_ps(nx), _mm_set1_ps(ny), _mm_set1_ps(nz));
    
    //const float m = 1.0f/sqrtf(nx*nx + ny*ny + nz*nz);
    const Vectorx4 wsNormal(_mm_set1_ps(nx*Aw), _mm_set1_ps(ny*Aw), _mm_set1_ps(nz*Aw));
    ...
    #pragma omp parallel firstprivate(i, prv_noHits)
	{
		...
		for(i = 0; i < noRays; i+=4) {
            ...
            azimuthal = _mm_sqrtApprox_ps(_mm_shuffle_ps(rand, rand, _MM_SHUFFLE(1,0,3,2)));
            zenith = _mm_mul_ps(azimuthal, _mm_shuffle_ps(rand, rand, _MM_SHUFFLE(2,1,0,3)));
            
            // S = A + sqrt(r1)*(B-A) + sqrt(r1)*r2*(C-B); where r1 and r2 are random numbers A, B and C are the triangle vertex
            wsRay.s._x = _mm_add_ps(_mm_add_ps(_mm_set1_ps(Ax), _mm_mul_ps(azimuthal, _mm_set1_ps(ABx))), _mm_mul_ps(zenith, _mm_set1_ps(BCx)));
            
            wsRay.s._y = _mm_add_ps(_mm_add_ps(_mm_set1_ps(Ay), _mm_mul_ps(azimuthal, _mm_set1_ps(ABy))), _mm_mul_ps(zenith, _mm_set1_ps(BCy)));
            
            wsRay.s._z = _mm_add_ps(_mm_add_ps(_mm_set1_ps(Az), _mm_mul_ps(azimuthal, _mm_set1_ps(ABz))), _mm_mul_ps(zenith, _mm_set1_ps(BCz)));
